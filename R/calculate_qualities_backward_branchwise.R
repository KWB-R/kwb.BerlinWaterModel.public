...
calculate_qualities_backward_branchwise <- function(input_list,
                                         flows,
                                         network,
                                         config,
                                         debug = TRUE) {
  ...
  if("backflows_multiple" %in% names(config)) {
    ...
    if(any(backflows_all_sections_defined$n != backflows_all_sections_defined$n_backflows)) {
      ...
      #message("do nothing as all upstream sections ar defined")
    }
  ...
  for(b_id in unique(df_orders$branch_id)) {
    ...
    message(sprintf("############### Calculating branch %d / %d",
                    b_id,
                    length(unique(df_orders$branch_id))))

    for(i in unique(df_orders_branch$id)) {
    ...
    message(sprintf("Calculating water quality 'forward' for section %s: %s",
     ...
     kwb.utils::catAndRun(messageText = sprintf("Calculating water quality 'forward' for branch_id: %d, section '%s: %s' (order_level: %d, id: %d)",
                                                 b_id,
                                                 get_ids_from_names(unique(df_order$order_name), config),
                                                 unique(df_order$order_name),
                           ...
                           expr = {
                             ...
                             if (b_id == 1 & i == min(df_orders_branch$id)) {
    ...
    # downstream_section_ids <- df_orders %>%
    #   dplyr::filter(section_id %in% sections_below_zero_and_upstream$section_id) %>%
    #   dplyr::pull(target_id) %>%
    #   unique()
    #
    # downstream_section_ids_to_recalculate <- downstream_section_ids[!downstream_section_ids %in% sections_below_zero_and_upstream$section_id] & downstream_section_ids %in% df_orders_branch$section_id]
      ...
      for(s_id in sections_below_zero_and_upstream$section_id) {
        ...
        if(length(names(quality_reverse$load_neg)) > 0) {
          ...
          for(load_neg_section_id in names(quality_reverse$load_neg)) {
            if(debug) {
            message(sprintf("Setting load neg for section_id == '%s' (%s)",
                            load_neg_section_id,
                            get_names_from_ids(load_neg_section_id, config)))
            }
