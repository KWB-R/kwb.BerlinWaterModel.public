...
cso <- if(reduce_temporal_coverage) {cso %>%
  dplyr::filter(datetime >= datetime_min,
...
inflows <- if(reduce_temporal_coverage) {
  inflows %>%
...
wwtp <- if(reduce_temporal_coverage) {
  wwtp %>%
    dplyr::filter(date  >= lubridate::as_date(datetime_min),
                  date <= lubridate::as_date(datetime_max))
} else {
...
wwtp <- if(no_ww_and_wwtp) {
  wwtp %>%
    dplyr::mutate(cbm_per_second = 0)
} else {
  wwtp
}
...
ww <- if(reduce_temporal_coverage) {
  ww %>%
    dplyr::filter(date  >= lubridate::as_date(datetime_min),
                  date <= lubridate::as_date(datetime_max))
} else {
...
shares_timeseries <- if(reduce_temporal_coverage) {
  shares_timeseries %>%
    dplyr::filter(date  >= lubridate::as_date(datetime_min),
                  date <= lubridate::as_date(datetime_max))
} else {
...
rain <- if(reduce_temporal_coverage) {
  rain %>%
    dplyr::filter(datetime  >= datetime_min,
                  datetime <= datetime_max)
} else {
